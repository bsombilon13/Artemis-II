    ...
    <div className={`relative h-full w-full flex items-center justify-center overflow-hidden transition-all duration-500 ${hideContainer ? 'bg-transparent' : 'glass rounded-xl p-4 border border-slate-800 bg-slate-950/40 min-h-[320px] md:min-h-[400px]'}`}>
      ...
      <div className="absolute inset-0 pointer-events-none z-10">
        ...
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 flex flex-col items-center z-50 pointer-events-none">
          {isBoosterSepFlash && (
            <div className="bg-orange-500/20 border border-orange-500 text-orange-400 font-bold px-4 py-1 text-[10px] tracking-widest rounded animate-pulse">
              Stage Event: SRB Jettison
            </div>
          )}
          {isLASSepFlash && (
            <div className="bg-blue-500/20 border border-blue-500 text-blue-400 font-bold px-4 py-1 text-[10px] tracking-widest rounded animate-pulse">
              Stage Event: LAS Jettison
            </div>
          )}
          {isCoreSepFlash && (
            <div className="bg-red-500/20 border border-red-500 text-red-500 font-bold px-4 py-1 text-[10px] tracking-widest rounded animate-pulse shadow-[0_0_15px_rgba(239,68,68,0.3)]">
              Stage Event: Core Separation Confirmed
            </div>
          )}
          {isOrionSepFlash && (
            <div className="bg-emerald-500/20 border border-emerald-500 text-emerald-400 font-bold px-4 py-1 text-[10px] tracking-widest rounded animate-pulse">
              Stage Event: Orion ICPS Separation
            </div>
          )}
        </div>
      ...
      <div className="absolute bottom-6 left-6 space-y-3 z-20 pointer-events-none">
        <div className="flex flex-col">
          <span className="text-[9px] text-slate-500 uppercase font-bold tracking-[0.2em]">Current Altitude</span>
        ...
        <div className="flex flex-col">
          <span className="text-[9px] text-slate-500 uppercase font-bold tracking-[0.2em]">Vehicle Velocity</span>
          <div className="flex items-baseline space-x-1">
      ...
      <div className="absolute top-6 right-6 text-right z-20 pointer-events-none">
        <div className="px-4 py-2 bg-slate-900/60 backdrop-blur-md border border-slate-800 rounded-lg shadow-xl">
           <div className="flex items-center justify-end space-x-2 mb-1">
              ...
              <span className="text-[10px] text-blue-300 font-bold uppercase">Staging Status</span>
           </div>
           <p className="text-[8px] mono text-slate-500 uppercase leading-tight font-bold">
             {isOrionSeparated ? 'Translunar Config' : isCoreSeparated ? 'ICPS Active' : isBoosterSeparated ? 'Core Sustainer' : 'Ascent Power'}
           </p>
        </div>
      </div>
