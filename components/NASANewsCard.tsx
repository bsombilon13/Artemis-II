    ...
    <div className="glass rounded-xl h-full border border-slate-800 flex flex-col overflow-hidden bg-slate-900/60 shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-950/20">
        <div className="flex flex-col min-h-full">
          ...
          {loading && updates.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-3 opacity-60 py-10">
              <div className="w-5 h-5 border-2 border-blue-500/20 border-t-blue-500 rounded-full animate-spin"></div>
              <span className="text-[9px] mono text-blue-400 uppercase tracking-[0.3em]">Acquiring Signal</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="h-8 bg-slate-900/95 px-3 border-t border-slate-800 flex justify-between items-center shrink-0">
        <div className="flex items-center space-x-3">
          <span className="text-[7px] text-slate-600 mono font-bold uppercase">Buffer: {updates.length}</span>
          <div className="w-px h-3 bg-slate-800"></div>
          <span className="text-[7px] text-slate-600 mono font-bold uppercase">Sync: {lastSync || '00:00:00'}</span>
        </div>
        <button 
          onClick={fetchNews}
          className="group flex items-center space-x-1 disabled:opacity-30"
          disabled={loading}
          title="Refresh Feed"
        >
